...
namespace Pycode {
  ...
  /* #endregion */
  const bs2i = (bytes: number[]) => {
    const last = bytes.length - 1
    return bytes.reduce((i, e, idx) => {
      i = idx == last ? i + e : (i + e) << 8
      return i
    }, 0)
  }
  export const pycode = (str: string) => {
    return str
      .split('')
      .map(e => {
        if (e.charCodeAt(0) < 129) {
          return e
        } else {
          const code = GBK.encode(e)
          switch (true) {
            case code[0] >= 170 && code[0] <= 254 && code[1] > 160:
              return GB2312.charAt((code[0] - 176) * 94 + (code[1] - 160) - 1)
            case code[0] >= 170 && code[0] <= 254 && code[1] <= 160:
              return GBK4.charAt((code[0] - 170) * 97 + (code[1] - 63) - 1)
            case code[0] <= 160 && code[1] >= 129:
              return GBK3.charAt((code[0] - 129) * 191 + (code[1] - 63) - 1)
            default:
              return e
          }
        }
